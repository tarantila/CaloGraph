...
restore_test_status_file=${BACKUP_RESTORE_TEST_STATUS_FILE:-$(dirname -- "$status_file")/restore-test.json}
restore_test_interval_days=${BACKUP_RESTORE_TEST_INTERVAL_DAYS:-90}
include_secrets=${BACKUP_INCLUDE_SECRETS:-false}
[[ "$restore_test_interval_days" =~ ^[0-9]+$ && "$restore_test_interval_days" -ge 1 && "$restore_test_interval_days" -le 365 ]] || { printf 'Invalid restore-test interval.\n' >&2; exit 64; }
