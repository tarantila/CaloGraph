...
TELEMETRY_EVENT = "analytics.weekly.canonical"
TELEMETRY_VERSION = "d4e.v1"
MAX_CANONICAL_READ_DAYS = 31


class WeeklyCanonicalTelemetryOutcome(StrEnum):
...
def _telemetry_outcome(outcome: DailyCanonicalOutcome) -> str:
    if outcome.state is DailyCanonicalState.MATCH:
        return (
            WeeklyCanonicalTelemetryOutcome.CANONICAL_SERVED
            if outcome.points is not None
            else WeeklyCanonicalTelemetryOutcome.CANONICAL_MATCH
        )
    if outcome.state is DailyCanonicalState.SKIPPED:
        return WeeklyCanonicalTelemetryOutcome.LEGACY_INELIGIBLE
    if outcome.state in {
        DailyCanonicalState.EXPECTED_DIFFERENCE,
        DailyCanonicalState.UNEXPLAINED_MISMATCH,
    }:
        return WeeklyCanonicalTelemetryOutcome.FALLBACK_PARITY
    if outcome.state is DailyCanonicalState.NOT_COMPARABLE:
        return WeeklyCanonicalTelemetryOutcome.FALLBACK_NOT_READY
    if outcome.state is DailyCanonicalState.FALLBACK:
        ...
        if outcome.reason is DailyCanonicalReason.LEGACY_SNAPSHOT_CHANGED:
            return WeeklyCanonicalTelemetryOutcome.FALLBACK_PARITY
        return WeeklyCanonicalTelemetryOutcome.FALLBACK_SEAL
    return WeeklyCanonicalTelemetryOutcome.ERROR
...
def run_weekly_canonical_read(
    ...
    *,
    ...
    request = CanonicalServingRequest(
        ...
        enabled=True,
        endpoint=CanonicalServingEndpoint.WEEKLY,
...
__all__ = [
    ...
    "TELEMETRY_EVENT",
    "TELEMETRY_VERSION",
    "WeeklyCanonicalTelemetryOutcome",
