...
TELEMETRY_EVENT = "analytics.trends.canonical"
TELEMETRY_VERSION = "d4g.v1"
MAX_CANONICAL_READ_DAYS = 31


class TrendsCanonicalTelemetryOutcome(StrEnum):
...
def _telemetry_outcome(outcome: DailyCanonicalOutcome) -> str:
    if outcome.state is DailyCanonicalState.MATCH:
        return (
            TrendsCanonicalTelemetryOutcome.CANONICAL_SERVED
            if outcome.points is not None
            else TrendsCanonicalTelemetryOutcome.CANONICAL_MATCH
        )
    if outcome.state is DailyCanonicalState.SKIPPED:
        return TrendsCanonicalTelemetryOutcome.LEGACY_INELIGIBLE
    if outcome.state in {
        DailyCanonicalState.EXPECTED_DIFFERENCE,
        DailyCanonicalState.UNEXPLAINED_MISMATCH,
    }:
        return TrendsCanonicalTelemetryOutcome.FALLBACK_PARITY
    if outcome.state is DailyCanonicalState.NOT_COMPARABLE:
        return TrendsCanonicalTelemetryOutcome.FALLBACK_NOT_READY
    if outcome.state is DailyCanonicalState.FALLBACK:
        ...
        if outcome.reason is DailyCanonicalReason.LEGACY_SNAPSHOT_CHANGED:
            return TrendsCanonicalTelemetryOutcome.FALLBACK_PARITY
        return TrendsCanonicalTelemetryOutcome.FALLBACK_SEAL
    return TrendsCanonicalTelemetryOutcome.ERROR
...
def run_trends_canonical_read(
    ...
    *,
    ...
    request = CanonicalServingRequest(
        ...
        enabled=True,
        endpoint=CanonicalServingEndpoint.TRENDS,
...
__all__ = [
    ...
    "TELEMETRY_EVENT",
    "TELEMETRY_VERSION",
    "TrendsCanonicalTelemetryOutcome",
