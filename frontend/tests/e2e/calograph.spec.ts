...
test('login, idempotent import and dashboard', async ({ page, request }) => {
  ...
  await expect(
    page.getByRole('heading', { name: /^(Willkommen bei CaloGraph|Ernährungsüberblick)$/ }),
  ).toBeVisible()
  const setupHeading = page.getByRole('heading', { name: 'Willkommen bei CaloGraph' })
  if (await setupHeading.isVisible()) {
    await expect(page).toHaveURL(/\/einrichtung$/)
    await expect(page.getByLabel('Kalorienbudget pro Tag')).toHaveValue('')
    await expect(page.getByLabel('Proteinziel pro Tag')).toHaveValue('')
    await expect(page.getByRole('navigation')).toHaveCount(0)
    await page.getByLabel('Kalorienbudget pro Tag').fill('2100')
    await page.getByLabel('Proteinziel pro Tag').fill('135')
...
test('administrator manages a complete synthetic account lifecycle', async ({ browser, page }) => {
  ...
  await expect(userPage.getByLabel('Kalorienbudget pro Tag')).toHaveValue('')
  await expect(userPage.getByLabel('Proteinziel pro Tag')).toHaveValue('')
  ...
  await userPage.getByLabel('Kalorienbudget pro Tag').fill('2100')
  await userPage.getByLabel('Proteinziel pro Tag').fill('135')
  ...
  await userPage.getByRole('link', { name: 'Wochen', exact: true }).click()
