...
test('login, idempotent import and dashboard', async ({ page, request }) => {
  ...
  if (await setupHeading.isVisible()) {
    ...
    await expect(page.getByLabel(/^(Kalorienbudget pro Tag|Daily calorie budget)$/)).toHaveValue('')
    await expect(page.getByLabel(/^(Proteinziel pro Tag|Daily protein target)$/)).toHaveValue('')
    await expect(page.getByRole('navigation')).toHaveCount(0)
    await page.getByLabel(/^(Kalorienbudget pro Tag|Daily calorie budget)$/).fill('2100')
    await page.getByLabel(/^(Proteinziel pro Tag|Daily protein target)$/).fill('135')
