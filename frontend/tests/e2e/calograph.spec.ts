...
test('login, idempotent import and dashboard', async ({ page, request }) => {
  ...
  await expect(
    page.getByRole('heading', { name: /^(Budgets & Ziele|Ernährungsüberblick)$/ }),
  ).toBeVisible()
  const setupHeading = page.getByRole('heading', { name: 'Budgets & Ziele' })
  if (await setupHeading.isVisible()) {
    await expect(page.getByLabel('Kalorienbudget')).toHaveValue('')
    await expect(page.getByLabel('Proteinziel (g)')).toHaveValue('')
    await page.getByLabel('Kalorienbudget').fill('2200')
    await page.getByLabel('Proteinziel (g)').fill('140')
    await page.getByRole('button', { name: 'Budget und Ziele speichern' }).click()
    await expect(page.getByRole('status')).toContainText('Budget und Ziele ab')
...
test('administrator manages a complete synthetic account lifecycle', async ({ browser, page }) => {
  ...
  await userPage.getByLabel('Benutzername').fill(lifecycleUsername)
  await userPage.getByLabel('Passwort').fill(initialPassword)
  await userPage.getByRole('button', { name: 'Anmelden' }).click()
  await expect(userPage.getByRole('heading', { name: 'Budgets & Ziele' })).toBeVisible()
  await expect(userPage.getByLabel('Kalorienbudget')).toHaveValue('')
  await expect(userPage.getByLabel('Gültig ab')).toHaveValue(/^\d{2}\.\d{2}\.\d{4}$/)
  await userPage.getByLabel('Gültig ab').fill('11.08.2026')
  await expect(userPage.getByLabel('Proteinziel (g)')).toHaveValue('')
  await userPage.getByLabel('Kalorienbudget').fill('2050')
  await userPage.getByLabel('Proteinziel (g)').fill('125')
  await userPage.getByRole('button', { name: 'Budget und Ziele speichern' }).click()
  await expect(userPage.getByRole('status')).toContainText('Budget und Ziele ab')
  await userPage.getByRole('link', { name: 'Übersicht' }).click()
