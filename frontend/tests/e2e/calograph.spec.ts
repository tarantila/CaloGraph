...
test('administrator manages a complete synthetic account lifecycle', async ({ browser, page }) => {
  ...
  await page.locator('select[name="language"]').selectOption('de')
  await page.locator('form:has(select[name="language"]) button[type="submit"]').click()
  await expect(page.locator('#account-center-title')).toHaveText('Einstellungen')
  ...
  await expect(userPage.getByRole('heading', { name: 'Persönliche Angaben' })).toBeVisible()
  await expect(userPage.getByText('Optionale Angaben kannst du überspringen und später jederzeit ändern.')).toBeVisible()
  await expect(userPage.getByRole('navigation')).toHaveCount(0)
  await userPage.getByRole('button', { name: 'Persönliche Angaben überspringen' }).click()
  await expect(userPage.getByRole('heading', { name: 'Ziele' })).toBeVisible()
  await expect(userPage.locator('input[name="calories-kcal"]')).toHaveValue('')
  await expect(userPage.locator('input[name="protein-g"]')).toHaveValue('')
  ...
  await userPage.locator('input[name="calories-kcal"]').fill('3000')
  await userPage.locator('input[name="maintenance-kcal"]').fill('2500')
  await userPage.locator('input[name="protein-g"]').fill('135')
  await userPage.getByRole('button', { name: 'Speichern und weiter' }).click()
  await expect(userPage.getByRole('heading', { name: 'Sicherheit' })).toBeVisible()
  await userPage.getByRole('button', { name: 'Vorerst überspringen' }).click()
  ...
  await userPage.getByRole('button', { name: 'Mit Passwort anmelden' }).click()
  await userPage.getByLabel('Benutzername').fill(lifecycleUsername)
  await userPage.getByLabel('Passwort').fill(initialPassword)
  await userPage.getByRole('button', { name: 'Anmelden' }).click()
  await expect(userPage).toHaveURL(/\/$/)
