...
function displayMatch(component: BackupComponentStatus | undefined): string {
  if (!component || component.matching_backup === undefined) return t('backupHealth.notReported')
...
<template>
  <section class="page-section">
    ...
    <section class="card admin-panel backup-health-overall" :aria-busy="loading" :aria-labelledby="'backup-health-heading'">
      ...
      <div class="admin-panel-header backup-health-header">
        <div>
          ...
          <p v-if="!loading && state !== 'healthy' && state !== 'unknown' && state !== 'disabled'" class="backup-health-reason">{{ reasonLabel }}</p>
        </div>
    ...
    <section class="card admin-panel backup-protection" aria-labelledby="backup-protection-heading">
